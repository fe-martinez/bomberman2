...
pub fn crear_pieza(s: &str, x_pos: usize, y_pos: usize) -> Tile {
    ...
    let tile = match primer_caracter {
        ...
        Some('B') => match segundo_caracter {
            None => Tile::Vacio,
            Some(numero) => Tile::BombaNormal(Bomba {x: x_pos, y: y_pos, radio: numero.to_digit(10).unwrap(), especial: false}),
        },
        Some('S') => match segundo_caracter {
            None => Tile::Vacio,
            Some(numero) => Tile::BombaEspecial(Bomba {x: x_pos, y: y_pos, radio: numero.to_digit(10).unwrap(), especial: true}),
        },
